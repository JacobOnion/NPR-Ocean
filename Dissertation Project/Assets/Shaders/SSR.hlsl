...
inline float2 ToRTUV(float2 uv01)
{
    ...
    // Stereo
    uv01 = UnityStereoTransformScreenSpaceTex(uv01);

    // RTHandle scale/clamp (RenderGraph uses RTHandles)
    // ClampAndScaleUVForBilinear is in Core.hlsl
    #if defined(_RT_HANDLE_SCALING_FACTORS)
    uv01 = ClampAndScaleUVForBilinear(uv01, _RTHandleScale.xy);
    #endif
    ...
    float stepScale, int maxSteps, float thickness, out float2 hitsUV)
{
   const int MAX_STEPS = 128;
    ...
    for (int i = 0; i < MAX_STEPS; i++)
    {
        ...
        float dz = abs(rayEyeZ - sceneEyeZ);
...
{
    float2 st = UnityStereoTransformScreenSpaceTex(screenPos);  // test w -wo
    //float st = ToRTUV(screenPos);
